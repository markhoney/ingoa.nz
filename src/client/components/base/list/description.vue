<template>
	<v-list three-line>
		<v-list-item v-if="search">
			<v-list-item-content>
				<v-list-item-title>
					<v-text-field label="Search" prepend-icon="search" solo v-model="term" />
				</v-list-item-title>
			</v-list-item-content>
		</v-list-item>
		<template v-for="item in (term ? data.filter(item => item.title.text.toLowerCase().includes(term.toLowerCase())) : data)">
