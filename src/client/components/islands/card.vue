<template>
	<v-card v-if="island">
		<nuxt-link :to="localePath({name: 'island-island', params: {island: localeCurrent(island.slug)}})" style="text-decoration: none;">
			<v-img v-if="island.images && island.images.landscape" :src="require(`~/assets${island.images.landscape}`)" height="160" class="white--text" style="padding: 20px; filter: grayscale(50%);" alt="">
