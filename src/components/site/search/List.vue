<template>
	<ul v-if="value && value.length >= 3" class="font-medium">
		<li v-for="result in searchResults" :key="result.id">
			<g-link :to="result.path" :title="result.index"><span v-html="boldNew(result.name.locale.mi || result.name.locale.en)" /></g-link>
		</li>
