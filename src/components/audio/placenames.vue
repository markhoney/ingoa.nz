<template>
	<section class="elevation-4 pa-5 my-3" v-if="current">
		<h3 :title="current.alt && current.alt.mi && current.alt.mi.phonetic ? current.alt.mi.phonetic.plain : ''" class="text-sm-center display-2">
			<g-link :to="localePath({name: 'placename-zone-placename', params: {zone: localeCurrent((current.placename.zone || current.placename.part || current.placename.island).slug), placename: localeCurrent(current.placename.slug)}})">
		...
		<h4 v-if="current.spoken.speaker" class="text-sm-center headline">
			...
			<g-link :to="localePath({name: 'speaker-speaker', params: {speaker: localeCurrent(current.spoken.speaker.slug)}})">
