<template>
	<div id="wave" class="my-3">
      ...
      <v-flex xs6>
				<v-slider v-model="volume" prepend-icon="volume_down" append-icon="volume_up" class="text-xs-right"></v-slider>
			</v-flex>
		</v-layout>
	</div>
</template>
		...
		for (var name in this.places[place].placenames) {
			if ('audio' in this.places[place].placenames[name]) {
				this.wavesurfer.addRegion({
					id: this.places[place].placenames[name].audio.code,
					start: this.places[place].placenames[name].audio.start,
					end: this.places[place].placenames[name].audio.end,
					attributes: {
						label: this.places[place].placenames[name].name,
						highlight: true
					},
					color: randomColor(),
					drag: false,
